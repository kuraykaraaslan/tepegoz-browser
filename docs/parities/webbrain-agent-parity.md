...
>   ring 200/tab; pure `summarizeConsole` shapes it. `level` filters by minimum severity; `levels`
>   tallies every observed message even when the reported tail is trimmed.
> - **Network** — a SECOND ring in `cdp-driver-network.electron.ts` (`networkRequestsSince`) alongside
>   the failure-only AI-8B ring: XHR/fetch/document requests, success AND failure, method/status/timing
