...
the **proposal half** — a real unified relevance score, favicons, inline autocomplete, keyword search
engines — none of it a defect, all of it owner-gated._ _(Tier 0 item 3, matched-substring emphasis,
landed 2026-09-09 as safe renderer-only polish — see § B.)_
...
| 3 ✅ | **Matched-substring emphasis** — _landed 2026-09-09_    | Chrome and Firefox both bold the typed span. Critical detail: folding is not index-preserving, so a fold→original index map is required — build it **once** in `@tepegoz/i18n`, not per call site. React children, never `dangerouslySetInnerHTML`.                                                                                                                                                           | S      |
...
> **Item 3 landed 2026-09-09** (`feat(omnibox): emphasize the matched substring in suggestions`). Both
> the primary line and the URL / secondary line now bold the part that matches what was typed.
>
> - New pure helper `emphasisSegments` / `emphasisRanges` in **`@tepegoz/omnibox`**
>   (`src/omnibox-emphasis.ts`), not `@tepegoz/i18n` as the proposal suggested — this iteration was
>   scoped to `packages/omnibox/**` only, and a shared `@tepegoz/i18n` fold→index map is still the
>   right home if a second call site ever needs it. The helper **reuses `foldForSearch`** from
>   `@tepegoz/i18n`, folding **per code point** and carrying each fold-space position's original UTF-16
>   offset, so the match maps back to original-string indices even though folding is not
>   length-preserving (`İ`→`i`, `Ş`→`s`). Turkish/accent behaviour matches how suggestions are already
>   filtered: `sisli` emphasises `Şişli`.
> - The suggestion row renders segments as plain `<span>` text nodes — matched runs carry
>   `font-semibold`, and there is **no `dangerouslySetInnerHTML`**. The row also gained an explicit
>   `aria-label` so a screen reader announces the whole title, not the emphasis-fragmented spans.
> - Edge cases covered by unit tests: no match, multiple / overlapping occurrences, query longer than
>   text, empty query, length-changing folds, plus a render test asserting the emphasis class and that
