...
Kaynak notu: Tablo, ilgili IETF/W3C standartları ve tarayıcı belgelerinin sentezidir. Özellikle DNS, TLS, ECH, QUIC, WebSocket, Service Worker, Beacon ve WebRTC için bkz. RFC 7858, RFC 8484, RFC 8446, RFC 6066, RFC 9848, RFC 9849, RFC 9000, RFC 9001, RFC 9114, RFC 6455, Service Worker ve Beacon spesifikasyonları. citeturn7search7turn7search2turn5search0turn6search1turn5search8turn19search4turn18search0turn21search1turn18search1turn17search2turn15search3turn4search6turn4search1
...
- Ağ katmanında varsayılan TLS 1.3, şifreli DNS desteği, ECH uyumluluğu ve sıkı sertifika doğrulama uygulayın; revocation tarafında istemci mahremiyetini bozan özel sorguları asgariye indirin. citeturn5search0turn7search2turn19search4turn33search6
- Cookie ve storage politikasını varsayılan olarak sınırlandırın: `Secure`, `HttpOnly`, `SameSite`, host-only önekler, partitioned state ve güçlü clearing semantics kullanın. citeturn35search4turn35search2turn35search6
- Üçüncü taraf state ve communication API’lerini top-level site’e göre bölümleyin; Storage Access istisnalarını dar ve denetlenebilir tutun. citeturn3search5turn35search6
- Renderer’ı sandbox içinde tutun; site isolation’dan geri adım atmayın; mobilde de mümkün olan en güçlü süreç izolasyonunu hedefleyin. citeturn36search0turn36search18turn36search19
- Prefetch/prerender ve background sync varsayılanlarını performans yerine mahremiyet lehine ayarlayın; çapraz site speculative traffic’i sınırlandırın. citeturn13search2turn20search3turn13search14
- Telemetriyi olay türüne göre sınıflandırın; URL-keyed ve teşhis verilerini ayrı izin katmanlarına bağlayın; retention ve scrub kuralları yazılı olsun. citeturn30search22turn30search12turn25view2
- Anti-fingerprinting’i liste tabanlı bloklamayla sınırlamayın; timer precision, canvas/audio/sensor yüzeyleri ve font/screen standardizasyonu gibi platform savunmaları ekleyin. citeturn15search2turn34search5turn29search17turn9search4
- Consent ve permission UX’ini hukuka uygun, dengeli ve geri alınabilir tasarlayın; kabul/reddet eşit görünürlükte olsun. citeturn26view0turn15search8
- WPT, fuzzing, privacy crawl ve adversarial tests’i CI/CD’ye koyun; privacy regressions’ı performans regressions kadar ciddiye alın. citeturn14search13turn14search0turn34search0

### Örnek başlık ve yapılandırma parçaları

Aşağıdaki parçalar **ürün politikasını anlatan örneklerdir**; doğrudan belirli bir tarayıcı iç API’sinin bire bir sözdizimi oldukları iddia edilmemektedir. Başlıkların kendisi standart davranışlara dayanır. citeturn14search16turn6search3turn31search3turn31search4
...
Bu örneklerde dikkat edilmesi gereken tasarım ilkesi şudur: header’lar ve cookie attribute’leri tek başına “privacy feature” değil, politika kodudur. Ürününüzde bunları yalnız web içeriğine bırakmak yerine, browser default’ları, enterprise policies ve per-site override’larla birlikte düşünün. Özellikle `CSP`, `Referrer-Policy`, `SameSite`, `Partitioned`, `COOP/COEP` ve permission yüzeyleri birbiriyle etkileşimlidir. citeturn6search5turn35search2turn31search14turn15search1

### Açık sorular ve sınırlamalar

Bu rapor platform bağımsız varsayımla yazıldığı için, bazı mobil platformlarda süreç izolasyonu, sandboxing kapsamı, ECH/DoH varsayılanları ve anti-fingerprinting davranışı masaüstü platformla bire bir aynı değildir. Özellikle mobilde site isolation ve network service süreç modeli ürün ve işletim sistemi kısıtlarına göre değişebilir. Ayrıca ECH dağıtımı 2026 itibarıyla bile tarayıcı, resolver, CDN ve enterprise policy kombinasyonuna bağlı olduğundan “her ağda aynı görünürlük azalması” beklenmemelidir. citeturn12search14turn36search19turn19search19turn19search6

## Kaynaklar

Aşağıdaki kaynaklar, bu konuyu derinlemesine çalışmak için en yüksek öncelikli referans kümesini oluşturur:

- **IETF RFC’leri ve standartları:** RFC 8446 (TLS 1.3), RFC 6066 (SNI), RFC 6960 (OCSP), RFC 6961 (OCSP stapling v2), RFC 7858 (DoT), RFC 8484 (DoH), RFC 9000/9001 (QUIC ve QUIC-TLS), RFC 9114 (HTTP/3), RFC 6455 (WebSocket), RFC 8828 (WebRTC IP handling), RFC 9848/9849 (ECH bootstrapping ve ECH). citeturn5search0turn6search1turn6search0turn33search0turn7search7turn1search2turn18search0turn21search1turn18search1turn17search2turn4search1turn5search8turn19search4
- **W3C / WHATWG dokümanları:** Service Workers, Beacon API, IndexedDB, Permissions, Permissions Policy, Referrer Policy, Fetch Standard, Device Orientation and Motion, Battery Status, fingerprinting guidance. citeturn15search3turn4search6turn4search2turn9search13turn3search4turn3search8turn3search24turn9search1turn9search0turn9search4
- **Chromium resmî belgeleri:** network stack, network service, multi-process architecture, resource loading, site isolation, sandbox, CT policy, RenderingNG, crash reports, UKM. citeturn12search2turn12search14turn16search3turn16search2turn36search0turn36search3turn12search0turn36search12turn30search1turn30search22
- **Firefox / Mozilla resmî belgeleri:** Necko/networking, Gecko, Fission/process model, DoH, ECH, private browsing, anti-fingerprinting, crash reports, Glean/telemetry, CRLite. citeturn13search0turn16search1turn36search2turn36search18turn7search2turn19search0turn3search3turn15search2turn30search12turn30search3turn33search2
- **Hukuk ve uyumluluk kaynakları:** GDPR resmi metni, ePrivacy Directive 2002/58/EC, EDPB 2024 Art. 5(3) kılavuzu, KVKK Çerez Uygulamaları Rehberi, CA/B Forum Baseline Requirements, Chrome ve Firefox CT politikaları. citeturn24view0turn24view3turn24view4turn10search1turn10search10turn12search0turn28search9
- **Seçilmiş akademik çalışmalar:** *How Unique Is Your Web Browser?*; *Browser Fingerprinting: A Survey*; *The Web Never Forgets*; *Long-Term Observation on Browser Fingerprinting*; *Process Separation for Web Sites within the Browser*; *Automated Discovery of Cross-Site Information Leaks in Browsers*; Web Audio fingerprinting ve Battery API mahremiyet çalışmaları. citeturn8search5turn8search1turn8search2turn8search13turn36search20turn8search12turn29search17turn9search19