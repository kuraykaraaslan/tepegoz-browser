...
| s16 | Validation-aware form-filling engine | 🟢 | [AI-4](phase-ai-4-action-vocabulary.md) — code landed: validation attrs (`required`/`pattern`/`aria-invalid`/…) now captured end-to-end + a `browser_validate_form` pre-submit gate (required-empty / flagged-invalid / visible errors). Typed-widget fill helpers + on-harness proof owed |
