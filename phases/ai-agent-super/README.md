...
| S1 | [phase-s1-foundation-native-loop.md](phase-s1-foundation-native-loop.md) | Native tool-calling + multimodal `CanonMessage` + streaming-to-UI (settled-to-Journal) | S0 | 🟡 **PR0–PR3 landed** 2026-08-18 (paired set + "before" exclusion rate frozen; `CanonMessage.content` widened to `string | CanonContentBlock[]`, schema in shared-types, `safeParse` at the gateway; anthropic/openai/gemini native both directions + `supportsNativeTools` (kimi + local stay JSON); ADR-0025 lands in PR5) |
