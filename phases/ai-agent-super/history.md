...
`phases/ai-agent-super/archive/`. This file preserves the two things that must **never** be lost even if
...
header *"ported from the browser-use / nanobrowser technique"* and runs in an **isolated world**.
...
`form_validation_required` **1/3** and `silent_api_failure` **1/3**. This invalidation is *why* the
...
both inherited here). v2's own M1 close condition pre-registered a humility clause: *"if the full
baseline disagrees with this plan's failure ranking, re-cut C1..C5 rather than defend the document."* The
