...
- [ ] **Candidate first real cheap-tier owner: intent → mode classification.** The mode-suggestion chip in
      [S8](phase-s8-assistant-ux.md) PR7 (from Opera Neon's Intelligent Mode) needs exactly one
      constrained token — `{chat|do|make|tasks}` — which a GBNF-grammar-constrained local call answers
      without a cloud round-trip. It is a good first candidate precisely because **a wrong answer is
      harmless**: the output is a _suggestion the user clicks_, never an automatic mode switch, so the
      cheap tier is being trusted with a recommendation rather than an action. Measure it in the sweep
      above like any other `SIMPLE_CAPABILITIES` candidate.
      [`../../docs/research-opera-neon.md`](../../docs/research/research-opera-neon.md).
...
>    scoring _higher_ is a reason to look at the exam, not a quality loss to guard against.
...
### PR6 — Tool-surface tiering for small models (parity extraction)

- [ ] **Size the tool surface to the model, not just the reasoning effort.** `EffortLevel` (`low..max`) tunes
      how hard the model _thinks_; nothing tunes how many tools it must _choose among_, and a large flat tool
      list is precisely what small local models handle worst. Add `compact` / `mid` / `full` tool tiers keyed
      to the selected model's capability — fewer tools for a small model, **still schema'd, still through the
      one PEP**. This is the constructive answer to the schema-less "quick mode" rejected in
      [S6](phase-s6-safety-control-plane.md) PR8: reduce the surface, never the typing.
      [`../tracks/webbrain-agent-parity.md`](../../docs/parities/webbrain-agent-parity.md) P8, with
      [`../tracks/kilocode-agent-parity.md`](../../docs/parities/kilocode-agent-parity.md) P3's `toolTierPreset` as a
      starting enum.
...
- **Local model too weak for exec, silently degrades quality.** **Mitigation:** the ±5pp equivalence margin is measured _before_ any tier is handed to local; `plan` stays frontier until proven; nothing over-promises (honesty-first, no vendor self-report anchoring).
