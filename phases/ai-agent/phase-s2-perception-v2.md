...
> **Where this came from.** [`../../docs/research/research-claude-for-chrome.md`](../../docs/research/research-claude-for-chrome.md),
...
### PR7 — Perception reach (competitor-parity extraction)

> **Where this came from.** The 25 competitor-parity tracks in [`../tracks/`](../../docs/tracks/README.md). Perception
> was the single most-repeated gap across them, and the convergences are the signal: **three independent
> tracks** named frame/shadow-DOM blindness, and **three more** named the two-weak-extractors problem. Each
> row cites the tracks that found it. Nothing here changes the ref model PR1–PR3 establish — this is _scope_,
> not a second perception system.

- [ ] **Frames and shadow DOM — the biggest single blind spot.** Perception is light-DOM-only today, so
      Web-Component-heavy sites (Stripe, Salesforce, Shopify are the ones named) are effectively invisible.
      Extend the **same** identity-stable ref model from PR1 to (a) enumerate `<iframe>`s as addressable
      targets — a frame is conceptually a nested `readPage`/`snapshotElements` scope, not a new tool family —
      and (b) pierce **open** shadow roots. Found by
      [`../tracks/webbrain-agent-parity.md`](../../docs/parities/webbrain-agent-parity.md) P3-b,
      [`../tracks/aipex-agent-parity.md`](../../docs/parities/aipex-agent-parity.md) P2 (which adds cross-origin frames
      merged into one tree, plus rich-editor content) and
      [`../tracks/stagehand-agent-parity.md`](../../docs/parities/stagehand-agent-parity.md) P3.
  - [ ] **Closed shadow roots and OOPIF** are the harder half; Stagehand's track argues for pulling them from
        "later" to "now" rather than shipping a reach that silently stops at the interesting cases.
- [ ] **A CDP-free DOM-snapshot fallback path.** `build-dom-tree-script` is already an injected DOM walk
      rather than a CDP AXTree dump, so this is mostly a **stated guarantee plus the gaps closed**: perception
...
- [ ] **A structural / landmark pass over the base tree.** Layer two things on the existing walk: structural
      context (headings, `<nav>`/`<main>`/`role="region"`, open dialogs) and a second pass for elements that
      are **cursor-interactive but ARIA-invisible** — a `role`-less `<div>` with a click handler or
      `cursor: pointer` that the a11y tree alone never surfaces.
      [`../tracks/browseros-agent-agent-parity.md`](../../docs/parities/browseros-agent-agent-parity.md) P3-b.
- [ ] **Disclosure-trigger hints.** Annotate elements that _reveal_ content when interacted with, so the agent
      knows a dropdown/accordion/"show more" exists rather than concluding the data is absent.
      [`../tracks/browserskill-agent-parity.md`](../../docs/parities/browserskill-agent-parity.md) P3.
- [ ] **Incremental rescrape for dynamically-populated widgets** — re-read only the changed subtree when a
      widget fills in asynchronously, instead of a full re-perceive or a fixed sleep.
      [`../tracks/skyvern-agent-parity.md`](../../docs/parities/skyvern-agent-parity.md) P2.
- [ ] **Schema-constrained structured extraction.** Let a caller ask for data **shaped like a schema** —
      "title, price and SKU of each result" — and get typed data back rather than prose the model must re-parse.
...
- [ ] **Let the agent ask what pages a site publishes.** `createSitemapReader` is already built and already
      ...
      (`AgentRunDeps.discoverSitemap`), never returned to the model — so "find the pricing page" still means a
...
- [ ] **One content extractor, not two weak ones.** `browser_get_article` and `web_get_page` run two separate,
      ...
      for Reading View — is better than both. Unify the agent-facing path onto it, add budget-aware trimming
      and clean-text/metadata serialization over its typed blocks. **Directly cuts token waste on every page
      read**, and writes no new extractor. Addendum to ADR-0008; found independently by
      [`../tracks/crawl4ai-agent-parity.md`](../../docs/parities/crawl4ai-agent-parity.md) P1,
      [`../tracks/firecrawl-agent-parity.md`](../../docs/parities/firecrawl-agent-parity.md) P1 and
      [`../tracks/steel-browser-agent-parity.md`](../../docs/parities/steel-browser-agent-parity.md) P1.
