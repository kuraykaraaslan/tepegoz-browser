...
**Every Comet-class rival ships memory that influences behaviour**, and north-star condition 4 ([README](README.md), [constitution](constitution.md#north-star--the-four-claim-conditions-full-text)) makes the repeat-domain cost drop a _published claim condition_ this program must satisfy — measured, not asserted. On the measured baseline ([eval-results.md](eval-results.md)) the DoD-model failures are on-page competence, and much of the wall-clock is re-perception of already-seen structure; a domain memory is the lever that turns a second visit cheaper.
...
Skills are **distinct from Phase 6 recipes** ([routing table](README.md#routing--what-stays-out), ownership test: _"if the model could be removed from the replay, it's Phase 6"_). A skill is a model-driven template (prompt + start URL + grant profile) that the agent still reasons over; a Phase-6 recipe is a signed, model-free deterministic replay. S9 ships the former only.
...
- [ ] **Prose:** none deleted (S9 owns no PROSE-LEDGER row); any future deletion this memory _enables_ is recorded when proven, never claimed here.
...
>    the evidence along with the attack. Quarantine also requires a _policy denial_, not mere task failure:
...
> 7. **Remembered grants cannot creep.** `expires_at` is `NOT NULL`, expiry is applied _in the query_ (so
...
- [ ] **Ship the library with templates in it — it is landed but empty.** All three rows above are done, so a
      ...
      YouTube transcript", "tell me when this price drops" (which hands off to a task). None of them needs a
      new capability — every one runs on tools that already ship. Include **a few Turkish-first templates**
      (e.g. sahibinden / trendyol price-watch), which double as fixtures overlapping the TR-web benchmark set
...
>    the transcript says _"Allowed by a permission you saved for X"_ where the approval would have been.
...
### PR7 — Skills: the next increment this phase's own text leaves open (parity extraction)

> S9 ships skills as **prompt templates only**, and says so in its own words. These rows are what the tracks
> propose building on top — each one crosses from "text that fills the composer" toward "a package that can
> declare capability", so each needs its own decision rather than arriving as an implementation detail.

- [ ] **Skills that declare tools.** A skill manifest that names HTTP/endpoint tools the skill needs, instead
      of only prompt text. **The trust question is the whole workstream:** a declared tool must be registered
      through the CapabilityRegistry, classified by `dangerClassFor`, and pass the one PEP exactly like a
      built-in — a skill may _request_ capability, never _grant_ it, and S9's "a skill can never start a run"
      rule is unaffected. [`../tracks/webbrain-agent-parity.md`](../../docs/parities/webbrain-agent-parity.md) P5.
- [ ] **OpenAPI-spec import as the front end for that manifest.** Paste a spec, get callable tools, pick an
      auth mode. Only meaningful once the row above exists; it is an authoring convenience over the same
      gated registration path, not a second one. Outbound calls re-enter the destination guard in
      [Phase 2](../product/phase-2-adapters-safe-browsing.md) L10.
      [`../tracks/librechat-agent-parity.md`](../../docs/parities/librechat-agent-parity.md) P3.
- [ ] **Trigger-based activation + tiers.** A skill only enters context when the model explicitly calls
      `load_skill` today. Trigger-based activation (a skill declares when it is relevant) saves the round-trip
      but hands the _prompt contents_ decision to a matcher — so it needs the same "trusted, never
      page-derived" rule as the site-guidance adapters in
      [Phase 2](../product/phase-2-adapters-safe-browsing.md) L6, and the active skill must be **visible** in
      the transcript ([S8](phase-s8-assistant-ux.md) PR8, B2).
      [`../tracks/openhands-agent-parity.md`](../../docs/parities/openhands-agent-parity.md) P3.
