...
- [ ] **Read `anthropic-quickstarts/computer-use-best-practices` before writing the eviction policy above.**
      The model vendor's own production-readiness repo enumerates the same list this phase is building —
      explicit tool definitions, image sizing and pruning, prompt caching, server-side compaction, trajectory
      recording, adaptive thinking/effort — and gives **numeric starting points** for image pruning and cache
      parameters, which is the part this phase would otherwise guess at. Two entries on its list are deliberate
      divergences worth stating rather than silently skipping: **batched tool calls** (the Executor here is
      serialized) and **sandboxed shell** (rejected — ADR-0026/0029). Reference only, no dependency; surveyed
      in [`../../docs/research-computer-use-agents.md`](../../docs/research/research-computer-use-agents.md).
- [ ] **Periodic tab/domain context reminder in long runs.** Re-state which domain the run is currently on
      into the model's context at intervals, rather than trusting the original grant text to survive a long
      window — Claude for Chrome does this, and it is close to the existing `runtime-context` envelope, so the
      cost is small. Pairs with the user-visible "the run changed site" event in
      [S8](phase-s8-assistant-ux.md) PR7: same fact, one shown to the model, one shown to the person.
      [`../../docs/research-claude-for-chrome.md`](../../docs/research/research-claude-for-chrome.md).

### PR7 — Context economy + caching (competitor-parity extraction)

> **Where this came from.** The parity tracks in [`../tracks/`](../../docs/tracks/README.md). Context management was
> the second-most-repeated gap after perception — **four independent tracks** landed on the same finding:
> `cache-window.ts`'s lag-2 breakpoints keep message _ordering_ cache-friendly, which is real, but nothing
> ever _reduces_ the context, and nothing tells the user it happened.

- [ ] **An explicit, visible mid-run compaction step.** When message count / characters / token budget crosses
      a threshold, summarize-and-continue and **show it** ("context automatically compacted") rather than
      silently reordering. The cache-window discipline stays exactly as-is underneath. Surfaced to the user by
      the context-fullness gauge in [S8](phase-s8-assistant-ux.md) PR8 (A2) — same event, two views.
      [`../tracks/webbrain-agent-parity.md`](../../docs/parities/webbrain-agent-parity.md) P9-a +
      [`../tracks/browseros-agent-agent-parity.md`](../../docs/parities/browseros-agent-agent-parity.md) P4 (mechanism) + [`../tracks/kilocode-agent-parity.md`](../../docs/parities/kilocode-agent-parity.md) P6 (numeric budget
      reference).
- [ ] **A model-generated summary as the _fallback_ tier, never the first one.** The deterministic paths —
      cache-window plus the Reactor's working-state collapse — are cheap and predictable and stay the default.
...
- [ ] **Intra-run action-result cache, model-in-the-loop on a miss.** Distinct from both existing mechanisms:
      ...
      model is back in the loop as normal, so it never becomes a stale-truth source.
      [`../tracks/stagehand-agent-parity.md`](../../docs/parities/stagehand-agent-parity.md) P2.
- [ ] **Agent-run-scoped resource blocking.** Block image/media/stylesheet loads _for the duration of an agent
      run_ — the agent reads the DOM, not the pixels — cutting bandwidth and page-load wall-clock. Must be
      **fail-closed toward [S10](phase-s10-vision-escalation.md)**: if a run escalates to vision, blocking has
      to be off, or the screenshot is of a page that never finished painting.
      [`../tracks/steel-browser-agent-parity.md`](../../docs/parities/steel-browser-agent-parity.md) P2.
