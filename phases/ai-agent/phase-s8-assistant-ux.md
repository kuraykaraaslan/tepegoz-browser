...
- [~] Step-feed component in `ext-agent` with per-step status (running/done/failed/skipped) driven by `step_*` events; split out of [panel-thread.tsx](../../extensions/ext-agent/src/panel-thread.tsx) if it approaches the 250-line cap. _`StepFeed` (`panel-step-feed.tsx`, 2026-09-08) — the collapsible per-turn progress feed, driven by the `step_start` / `step_ok` / `step_error` events already on the turn, coloured by `KIND_DOT`, header count + latest-step-inline while working. `panel-thread.tsx` back to 239 lines. **`running` now distinct:** the trailing in-flight `step_start` (last step, no `step_ok`/`step_error` after it) shows a pulsing amber dot while the turn is working, so "where is the run" reads at a glance. 8 tests (`panel-step-feed.test.tsx`). **Still owed for `[x]`:** `skipped` — a plan step the user skipped produces **no** `step_*` event, so the event-driven feed cannot show it; that belongs to the plan-preview surface (which already lists skips), and is why this stays `[~]`._
