...
The claim is **inherited verbatim** from v2 and stays binding. tepegoz claims _world's best browser
agent_ **only** when, at a dated release, **all four** hold (full text in [`constitution.md`](constitution.md)):
...
2. **Bounded, honest injection ASR** — published as _"k successes in K trials, 95% binomial upper bound
   X%"_, upper bound ≤5% initially. Owned by [S6](phase-s6-safety-control-plane.md).
...
The owner's verdict — _"hala istediğim gibi çalışmıyor"_ — decomposes into four confirmed pains. Each
...
| --------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| **M1** measurement backbone       | `e01691b`, `f9e639d` (merge `715a10e`) — cost, wall-clock, Wilson CIs, honest scenarios, read cap, per-tag pooled family aggregates                             | **none** — this _is_ the measuring instrument, not a capability claim | **absorbed**, no debt                                                                                |
...
as separate debt because both are discharged by the _same_ S0 full-registry baseline.
...
compliance.** Until then, treat every capability in S1–S3 as _landed and deterministically tested_,
never as _measured_.
...
is discharged by unit + regression tests, not by a sweep. A phase only enters 🟠 when a _sweep_ is owed,
...
| _Reserve_ (overrun, re-runs, transport-invalid retries)            |           |                         |                                     **~$6** |
...
keeps history compact, _and_ it mutates messages in place, so a tail breakpoint would be invalidated
...
agent loop (1.5B/3B weights at 8192 ctx) — it is a cheap-_capability_ track, not a free eval tier.
...
| Stays with                                                         | Material                                                                                                                                                                                                                            |
| ------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [Phase 6](../product/phase-6-deterministic-automation.md)          | Deterministic **model-free** signed recipes, self-healing selectors, success oracle. Ownership test: _"if the model could be removed from the replay, it's Phase 6."_ S9's skills are model-driven templates, not recipes.          |
| [Phase 8](../product/phase-8-local-intelligence-sovereignty.md)    | Provider trust mesh, knowledge graph, **learned** ModelRouter, speculative two-tier. S12 ships a _static_ local tier, not a learned router.                                                                                         |
