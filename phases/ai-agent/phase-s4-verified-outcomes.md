...
downgraded to _"attempted, unverified"_. Add a deterministic **URL re-verification** before mutating
...
_fabricated-success_ — in [constitution.md](constitution.md) (§North star, item 3): _"on trap fixtures
where the page lies about success, the agent reports the truth."_
...
has no _verified-completion_ or _fabricated-success_ metric, so condition 3 is currently unmeasurable
...
- [x] Author the backing fixtures under `test-fixtures/sites/` served by
      ...
      truth encodes the honest answer (e.g. _"not saved; server returned 500"_) via `success.expectedValue`.
...
> comparable. A new file was rejected because the DoD pools _this_ family, and splitting it would leave
...
>    lying is precisely the failure this phase removes, so the model is left with the _wording_ only.
...
>    verify. Without it, requiring evidence would downgrade every honest read to _unverified_ and the
...
>    here is "am I still on the same _site_?", and a URL origin answers it exactly while a content hash
...
>    driver owns — the _rule_ is pure and unit-tested in `origin-guard.ts`, only its application is in main.
...
> 2. **Fabricated success needs no per-scenario "is this a trap" tag.** It is _claimed done_ × _ground
>    truth says wrong_, which is computable for every scenario in the registry — so the metric covers the
...
**Owns [PROSE-LEDGER.md](PROSE-LEDGER.md) row 6** — _"Read `networkWarning`; don't `web_search` to
confirm your own save."_ Deleted in PR4 (same PR as the proving paired sweep), because the validator now
...
- **Honest-but-annoying "cannot verify" outcomes.** Requiring evidence will make the agent refuse to
  ...
  denominator. _Mitigation:_ the DoD counts cannot-verify as a **separate terminal category** (PR3), and
...
- **False-swap refusals from benign same-site redirects** (login → app, `www` → apex). _Mitigation:_
...
- **Network recorder gaps** (a mutating request the CDP recorder misses ⇒ no verdict ⇒ downgrade to
  unverified). _Mitigation:_ acceptable failure direction — absence of evidence yields _unverified_, not
  _fabricated success_; the DoD gate is on fabricated-success = 0, which this bias protects.
