...
- [ ] **Cross-check the recovery taxonomy against Amazon Nova Act's, which is cut on a different axis.** The
      ...
      unparseable model output, max steps — the _user_ can retry with a different request),
      `ActExecutionError` (a valid decision failed locally during actuation), `ActClientError` (the request
      itself was invalid — guardrail block, rate limit), `ActServerError` (service-side). Neither axis is
