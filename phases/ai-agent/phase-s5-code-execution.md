...
### PR4 — Schema-shaped extraction scaffold (parity extraction, gated behind this phase reaching ✅)

- [ ] **Let a caller declare the shape it wants and get typed data back.** AgentQL's one genuinely good
      ergonomic idea is a query that names the fields (`{ title, price, sku }`) rather than prose the model
      must re-parse. Built **inside this phase's existing extraction sandbox contract** — the scaffold shapes
      and validates the output, it does not add a new execution plane, and it does not reopen ADR-0026. The
      perception half (finding the elements) is [S2](phase-s2-perception-v2.md) PR7's schema-constrained
      extraction row; this is the typing half. Strictly gated: nothing here starts before this phase's own
      sandbox contract is ✅.
      [`../tracks/agentql-agent-parity.md`](../../docs/parities/agentql-agent-parity.md) P1 +
