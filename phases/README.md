...
| 5 | [phase-5-vpn-network-privacy.md](product/phase-5-vpn-network-privacy.md) | Per-tab & per-group VPN tunnels + Tor (network privacy) | 🟡 In progress (decision layer + **enforcement seam** landed: per-session wiring registry, fail-closed egress config with no `DIRECT` fallback, verified `setProxy` call site, and a **passing leak test** against the shipping app. Also closed four unlisted clear-path escapes: popups, tab-strip favicons, tab creation, and the app's own HTTP. No connection pool, no provider, no UI — every tab is still Direct) |
