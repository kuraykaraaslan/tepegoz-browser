...
| 5 | [phase-5-vpn-network-privacy.md](product/phase-5-vpn-network-privacy.md) | Per-tab & per-group VPN tunnels + Tor (network privacy) | 🟡 **5a working** — connection pool + three-scope binding + reload-on-switch + native route pickers + Settings surface + per-tab indicator, measured end-to-end in the shipping app (routes a tab through a live SOCKS endpoint; kills it and the tab fails closed while a proven-reachable clear path records nothing). Routes through a SOCKS5 endpoint the user already runs; **bundled WireGuard/Tor is blocked on Phase 0 code-signing**, 5b on Phase 3 |
