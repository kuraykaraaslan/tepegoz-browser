...
- [ ] **i18n:** en+tr keys added for new surfaces (timeline/snapshots, Tidy preview, highlight/research canvas
      + export, onboarding import digest, trust-lens badges + Güven Turu)
...
- [ ] *Risk:* restoring authenticated tabs could re-open sensitive sites → re-apply sensitive-site lockout +
...
- [ ] *Risk:* never auto-suspend tabs with **dirty form state** (detectable via CDP); the dry-run preview means
...
- [ ] *Risk:* highlights/exports may carry PII → local embedding (no PII leaves device), sensitive-site lockout
      + per-site opt-in, export runs redaction + HITL confirmation before writing to disk
...
- [ ] *Risk:* imported history is sensitive → processed only on-device, import opt-in, digest is a local
...
- [ ] *Risk:* reputation hints are heuristic/labeled (no false "safe" guarantee per the residual-risk DNA);
