...
- [ ] *Risk:* replay fidelity depends on recording all non-deterministic inputs as observation events; fixtures
...
- [ ] *Risk (ADR-0020):* community-authored state-changing automations are an abuse vector → deterministic-only
...
### L5/L7 — Capability Marketplace economics (runs-locally paid skills)  *(adoption-gated)*
...
- [ ] *Risk (ADR-0021):* entitlement bends local-first for PAID items → free items never touch backend; paid
