...
| [developer-settings-surface.md](developer-settings-surface.md) | 🚧 **In progress** — Tier B shipped 2026-08-28 | Grow the dev-only **Developer** section of `tepegoz://settings` into one place for every knob: enriched `Preferences` table (nested drill-down + schema-driven metadata — owed), allowlist-only Chromium flags (`chrome://flags` analog — **shipped**), and a safe subset of per-tab `webPreferences` / `session` defaults (owed). Renderer-boundary keys (`contextIsolation`/`sandbox`/`nodeIntegration`/`webSecurity`) stay permanently locked. Companion: [ADR-0041](../../docs/adr/0041-developer-settings-surface.md). |
