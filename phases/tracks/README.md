...
| [developer-settings-surface.md](developer-settings-surface.md) | 📝 **Planned** — unscheduled, no owner | Grow the dev-only **Developer** section of `tepegoz://settings` into one place for every knob: enriched `Preferences` table (nested drill-down + schema-driven metadata), allowlist-only Chromium flags (`chrome://flags` analog), and a safe subset of per-tab `webPreferences` / `session` defaults. Renderer-boundary keys (`contextIsolation`/`sandbox`/`nodeIntegration`/`webSecurity`) stay permanently locked. Companion: [ADR-0041](../../docs/adr/0041-developer-settings-surface.md). |
