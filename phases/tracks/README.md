...
| [protocol-tepegoz-pages.md](protocol-tepegoz-pages.md) | 🟡 **In progress** — Faz 0/1/2 done (2026-08-26) | Serving `tepegoz://` internal pages (settings first) as real pages via `protocol.registerSchemesAsPrivileged` + `protocol.handle` — socketless. `tepegoz://settings` is now a real `WebContentsView` with a working native context menu, root-causing and working around a real Electron bug along the way (subresource requests never reach a `protocol.handle`-on-named-session handler; the fix inlines the whole bundle into one self-contained document). Faz 3 (other internal pages) not started. |
