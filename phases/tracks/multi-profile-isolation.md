...
active profile, not the calling window's. That is the *class* of bug the design invites — correctness
...
does *not* do this). Deleting a running profile is refused with an actionable message (files locked);
...
- A profile switch must not leak the previous profile's in-memory secrets into the new process — with
  process-per-profile this is automatic (fresh process), which is a security argument *for* that design.
