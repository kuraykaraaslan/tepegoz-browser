 ...
 *
 * Keyed by `(profile, connection)`, deliberately NOT by group — per the phase's own model, groups are a
 * binding/UI layer, and N groups sharing one connection share one partition. A `Direct` resolution uses
 * the plain profile partition (no `--conn-` suffix), so untouched tabs are byte-identical to how the
 * browser already partitions profiles today — Phase 5 adds partitions, it does not change the existing
 * default one.
 */
export function partitionKeyFor(profileId: string, resolved: ResolvedConnection): string {
  return resolved.connectionId === null
    ? `persist:tepegoz-profile-${profileId}`
    : `persist:tepegoz-profile-${profileId}--conn-${resolved.connectionId}`;
}
