 ...
 *
 * In-memory, PER-RUN accumulator: {@link reset} is called at the start of every run so the live
 * indicator shows this task's spend. The host persists {@link entries} to the SQLite Token Ledger at
 * run end (provider+model+capability granularity, sync-ready) and seeds {@link setBaseline} with the
 * account's lifetime total so the {@link budgetStatus} quota/80%-warning reflects cumulative usage,
...
export class TokenLedger {
  ...
  static setBaseline(usedTokens: number): void {
    TokenLedger.baseline = Number.isFinite(usedTokens) && usedTokens > 0 ? usedTokens : 0;
  }
  ...
  static setQuota(totalTokens: number): void {
    TokenLedger.quota = Number.isFinite(totalTokens) && totalTokens > 0 ? totalTokens : 0;
  }
  ...
  static budgetStatus(): BudgetStatus {
    const used = TokenLedger.baseline + TokenLedger.totals().totalTokens;
    const quota = TokenLedger.quota;
  ...
  static quotaExhausted(): boolean {
    return TokenLedger.quota > 0 && TokenLedger.baseline >= TokenLedger.quota;
  }
