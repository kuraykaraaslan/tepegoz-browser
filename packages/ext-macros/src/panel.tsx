...
import { MACRO_IR_VERSION, type Macro, type MacroRunProgress, type MacroSummary, type Step } from '@tepegoz/shared-types';
...
const ADDABLE_KINDS = ['waitMs', 'navigate', 'waitFor', 'waitLoad'] as const;
...
function newStepOfKind(kind: AddableKind): Step {
  switch (kind) {
    ...
    case 'waitFor':
      return { kind: 'waitFor', target: [{ kind: 'css', value: '' }], timeoutMs: DEFAULT_ELEMENT_TIMEOUT_MS };
    case 'waitLoad':
...
function MacrosCore({ api }: Readonly<{ api: MacrosHostApi }>) {
  ...
  const stepTypeLabel = (kind: AddableKind): string =>
    kind === 'waitMs'
      ? t.stepWaitMs
      : kind === 'navigate'
        ? t.stepNavigate
    ...
    if (step.kind === 'fill') {
      return (
        <span className="flex min-w-0 flex-1 items-center gap-1">
          <span className="truncate font-mono text-text-secondary">{step.target[0]?.value ?? ''}</span>
          <input
            type="text"
            value={step.value}
