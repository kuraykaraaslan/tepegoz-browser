...
const pool = vi.hoisted(() => ({
  connections: [] as { id: string; label: string; status: string }[],
}));
vi.mock('../network/connection-pool.electron', () => ({
  default: { list: () => pool.connections },
}));
...
describe('routeSubmenu', () => {
  ...
  it('lists each connection with its health spelled out, and checks the bound one', () => {
    ...
    expect(labels(items)).toEqual(
      expect.arrayContaining(['Sweden — connected', 'Iceland — connecting']),
    );
  ...
  it('clicking a connection binds that scope to it', () => {
    ...
    expect(binding.bindGroup).toHaveBeenCalledWith('g9', {
      kind: 'connection',
      connectionId: 'c1',
    });
  });
