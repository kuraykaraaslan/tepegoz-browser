...
describe('the notification / HITL-prompt listeners forward only their payload', () => {
  it.each([
    ...
    [
      ...
      (cb: (p: unknown) => void) => api.onNotificationsState(cb),
      { unread: 2 },
    ],
    [
      'onNotificationToast',
      (cb: (p: unknown) => void) => api.onNotificationToast(cb),
      { id: 't1', title: 'Hi' },
    ],
    [
      'onNotificationPermissionRequest',
    ...
    [
      'onBasicAuthRequest',
      (cb: (p: unknown) => void) => api.onBasicAuthRequest(cb),
      { requestId: 'a1', host: 'x.test' },
    ],
    [
      'onCertificateErrorRequest',
